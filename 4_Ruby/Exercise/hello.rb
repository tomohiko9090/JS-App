...
p school[0, 2] #カンマの後ろは要素数でスライスできる
p school[0]
p school[0, 1]
p school[1..3] #3番目の要素も表示する
p school[1...3] #3番目の要素は表示しない→違う挙動！

puts "\nテスト6"
puts school << "5_大学院" #これでも要素追加できる
...
puts "\nテスト13"
# シンボル
fruits = {apple: "りんご", orange: "みかん", lemon: "レモン", grape: "ぶどう" } #整数の番号で管理してるから動作が早いらしい(object_idやね)
...
Cat.shukai #これがクラスメソッド！インスタンスかしていないよね？
...
puts hanako.name #あたかもインスタンス変数に代入したかのようにふるまってくれる！
puts hanako.age


puts "\nテスト38"
# アクセスメソッドを使えば、簡潔にテスト37と同じ記述ができる
class Human
...
puts "\nテスト39"
# アクセス権限。プライベートにしたら、継承先からならアクセウスできるはず。
class AccessTest
...
puts "\nテスト63"
str.match(/Ruby/)
p ":019A=zfdsafdss\.".match(/:[0-3][0-9][0-9][a-zA-Z]+=[a-z]*\./)

puts "\nテスト64"
place = "東京都"
p /#{place}/.match("Go to 東京都")
